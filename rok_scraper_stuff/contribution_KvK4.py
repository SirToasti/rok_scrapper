...
def parse_profile_with_kills_screenshot(raw_image):
    ...
    width2, height2, = kills_crop.size
    total_crop = trim_to_bbox(kills_crop.crop((width2 * 0.20, 0, width2 * .5, height2 * 0.12)))
...
def process(date, start=1, end=800):
    # path = r'C:\Users\lawre\PycharmProjects\RoK\power\assets\contribution_samples\new'
    path = r'E:\RoK\2020_KvK4\contribution\screenshots\{}'.format(date)
    output_file = path + '.csv'

    kills_pattern = re.compile('(\d+)_kills.png')
    moreinfo_pattern = re.compile('(\d+)_moreinfo.png')
    name_pattern = re.compile('(\d+)_name.txt')

    with open(output_file, 'a', encoding='utf-8', newline='') as csvfile:
        fieldnames = ['needs_review', 'id', 'name', 'power', 'kill points', 't5 kills', 't4 kills', 't3 kills', 't2 kills', 't1 kills', 'deads', 'rss gathered', 'rss assistance', 'helps']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        for count, (kills_path, more_info_path, name_path) in enumerate(zip(*[iter(glob.glob(path + r'\*.*'))]*3)):
            if count < start - 1:
                continue
            if count >= end:
                break
            ...
            name_match = name_pattern.search(name_path)
            assert kills_match and moreinfo_match and name_match
            assert kills_match.group(1) == moreinfo_match.group(1) == name_match.group(1)
            id = name_match.group(1)
            with open(name_path, 'r', encoding='utf-8') as name_file:
                name = name_file.read()
            print('{}: parsing person {:3d}: {} {}'.format(datetime.now().strftime('%H:%M:%S'), count + 1, id, name))
...
process('2021-09-28', start=1, end=1)
